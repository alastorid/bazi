...
ORDER BY ${order ?? `r."${NAMES[dimension]}分" DESC, m."公曆日期", m."時辰序號", m."性別"`};`;
  ...
  const definitions = [
    ...
    q("wealth_great","wealth","巨富格","武貪權祿坐命、狹義巨日格或祿馬交馳。",{dimension:"wealth",join:G,fields:['m."命宮主星"','m."財帛主星"','g."武貪權祿坐命"','g."巨日格"','g."祿馬交馳"',...hua],where:'g."武貪權祿坐命"=1 OR g."巨日格"=1 OR g."祿馬交馳"=1'}),
    ...
    q("wealth_quanlu","wealth","權祿組合","財帛權祿、命宮三方權祿與官祿權財。",{dimension:"wealth",join:G,fields:['g."權祿同財帛"','g."權祿相逢命"','g."官祿權財"',...hua],where:'g."權祿同財帛"=1 OR g."權祿相逢命"=1 OR g."官祿權財"=1'}),
    ...
    q("romance_top","appearance","桃花 SSS～S","紅鸞、天喜、貪狼及太陰昌曲組合。",{dimension:"romance",join:G,fields:['m."命宮主星"','m."命宮全部星"','g."泛水桃花"','g."太陰昌曲同命"'],where:topRank("桃花")}),
    ...
    q("marriage_best","marriage","婚姻有利組合","天府天馬、廉府或權祿同在夫妻宮。",{dimension:"marriage",join:G,fields:['m."夫妻主星"','m."夫妻全部星"','g."天府天馬同夫"','g."廉府同夫妻"','g."權祿同夫妻"'],where:'g."天府天馬同夫"=1 OR g."廉府同夫妻"=1 OR g."權祿同夫妻"=1'}),
    ...
    q("pattern_rare","pattern","罕見大格","紫府坐垣、七殺朝斗、雄宿朝元、狹義巨日與祿馬交馳。",{dimension:"lucky",join:G,fields:['m."命宮"','m."命宮主星"','g."紫府坐垣"','g."七殺朝斗"','g."雄宿朝元格"','g."巨日格"','g."祿馬交馳"'],where:'g."紫府坐垣"=1 OR g."七殺朝斗"=1 OR g."雄宿朝元格"=1 OR g."巨日格"=1 OR g."祿馬交馳"=1'}),
    ...
    raw("pattern_catalog","pattern","格局規則目錄","檢查正式格局、強組合、條件組合、稀有度及是否可計分。",`SELECT TOP 1000
  "名稱", "類型", "嚴格度", "結構稀有度", "稀有度說明", "吉凶",
  "完整解釋", "可計算", "可計分", "相關星曜", "成格條件", "教材結果"
FROM "格局規則"
ORDER BY "可計分" DESC, "結構稀有度" DESC, "名稱";`),
    raw("relationship_pattern_catalog","pattern","兩盤關係格目錄","母子格等屬兩人命盤對應，不進單張命盤排名。",`SELECT TOP 1000
  "名稱", "類型", "適用範圍", "完整解釋", "條件說明"
FROM "關係格局規則"
ORDER BY "完整解釋" DESC, "名稱";`),
