...
try {
  ...
  await expect(page.locator('.timing-period')).toHaveCount(24,{timeout:120000});
  ...
  await expect(page.locator('.compare-verdict')).toContainText('乙較前',{timeout:120000});
  const key=await page.locator('#compareA').inputValue();
  await page.locator('#compareB').fill(key);
  await page.locator('#compareRun').click();
  await expect(page.locator('.compare-verdict')).toContainText('相同',{timeout:120000});
  await page.locator('#compareB').fill('不存在');
  await page.locator('#compareRun').click();
  await expect(page.locator('#compareState')).toContainText('找不到命盤',{timeout:120000});
  await page.locator('[data-workspace-tab="database"]').click();
