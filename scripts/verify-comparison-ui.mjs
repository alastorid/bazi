...
try {
  ...
  await page.evaluate(async()=>{
    ...
    charts[0].吉宮數=3;charts[0].評選合格=1;charts[0].壯年吉限年數=20;charts[0].壯年風險年數=0;charts[0].首次吉限年齡=22;
    charts[1].百分位=null;charts[1].吉宮數=3;charts[1].評選合格=0;charts[1].壯年吉限年數=0;charts[1].壯年風險年數=0;charts[1].首次吉限年齡=62;charts[1].晚發限定=1;charts[1].未達門檻原因='壯年吉限不足';
    ...
    await window.BAZI_COMPARISON.init({showWorkspace(){},query:async(sql,params=[])=>{
      ...
      if(sql.includes('命盤大限明細'))return {rows:charts.filter(c=>params.includes(c.KEY)).flatMap(c=>names.map((宮位,i)=>({KEY:c.KEY,宮位,起始年齡:2+i*10,終止年齡:11+i*10,判讀:i<3?'吉':'平',得時作用:i<3?10:0,年齡權重合計:10,壯年吉限年數:i===2?10:0,壯年風險年數:0,依據:'合成資料之明示依據'})))};
  ...
  await expect(page.locator('.compare-verdict')).toContainText('乙較前');
