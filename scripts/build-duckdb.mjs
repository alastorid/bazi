...
const queries = shard?[]:window.BAZI_QUERY_LIBRARY.definitions.map(q=>({name:q.key,sql:translateTop(q.sql)}));
for (const gender of shard?[]:['女','男','女＋男']) {
...
staging.close();
const result = spawnSync('python3', ['scripts/build-duckdb.py','data/duckdb-checks.json',...(shard?['--shard']:[])], {stdio:'inherit'});
if (result.status !== 0) process.exit(result.status ?? 1);
const nativeMetadata=JSON.parse(fs.readFileSync('data/metadata.json','utf8'));
nativeMetadata.dataFingerprint=nativeFingerprint;
fs.writeFileSync('data/metadata.json',JSON.stringify(nativeMetadata,null,2)+'\n');
